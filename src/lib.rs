...
impl<T, const A: u8, const S: bool, const V: u8> Ointer<T, A, S, V> {
  ...
  pub fn as_ptr(self) -> *mut T { unsafe { unpack(self.ptr, A, S, V) } }
...
impl<T, const A: u8, const S: bool, const V: u8> Ox<T, A, S, V> {
  ...
  pub fn into_box(self) -> Box<T> { unsafe { Box::from_raw(unpack(self.as_ptr(), A, S, V)) } }
...
impl<T, const A: u8, const S: bool, const V: u8> Deref for Ox<T, A, S, V> {
  ...
  fn deref(&self) -> &T { unsafe { &*self.0.as_ptr().cast() } }
}

#[cfg(feature = "alloc")]
impl<T, const A: u8, const S: bool, const V: u8> DerefMut for Ox<T, A, S, V> {
  fn deref_mut(&mut self) -> &mut T { unsafe { &mut *self.0.as_ptr().cast() } }
}

#[cfg(feature = "alloc")]
impl<T, const A: u8, const S: bool, const V: u8> Drop for Ox<T, A, S, V> {
  fn drop(&mut self) { drop(unsafe { Box::from_raw(self.0.as_ptr()) }) }
}


/// Packs a pointer into the bottom `sizeof(usize) - (a + s + v)` bits of a
/// usize.
///
/// # Safety
///
